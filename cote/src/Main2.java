...
//			System.out.println(Arrays.toString(A));	// 입력받은 값을 출력해봄
			// 여기에  알고리즘 포함
			int score_sum = 0;		// 같은 줄 테스트 케이스의 팀의 실력의 최대 합을 계산후 절댓값을 취해서 더한 값
			for(int i=1; i<=N; i++) {
				int team_score_max = -2147483648;		// i번째 선수가 포함된 팀의 점수 중 가장 큰 값을 저장
				for(int s=1; s<=i; s++) {
					for(int t=i; t<=N; t++) {
						int i_score = 0;				// i번째 선수가 포함된 팀의 점수를 저장
						int min = 10001;				// 선수들의 실력중 가장 큰값이 10000이므로 그보다 1큰 값을 min으로 초기화
						for(int j=s; j<=t; j++) {
//							System.out.print(j + " ");
							i_score = i_score + A[j];	// i번째 선수가 포함된 팀의 점수를 계산
							if(j!=i && A[j]<min) {
								min = A[j];				// 가장 작은 값을 저장
							}
						}
...
//						System.out.print(i_score+ " ");	// i번째 선수가 포함된 팀의 점수를 출력
						if(i_score > team_score_max) {	// 각 선수별 팀 실력이 가장 큰 값을 저장
							team_score_max = i_score;	// 팀의 점수중 가장 큰 점수를 저장
						}
					}
				}
				System.out.print(team_score_max + " "); // i번째 선수가 포함된 팀의 점수 중 가장 큰 값 저장
